...
def find_location_mentions(query:str):
    ...
    for iata, airport in AIRPORTS.items():
        airport_city = str(airport.get("city", "")).lower().strip()
        # If the city name is valid and written as a standalone word in the query
        if airport_city and len(airport_city) >= 3: 
            if re.search(rf"\b{re.escape(airport_city)}\b", q):
                mentions.append(airport_city)
